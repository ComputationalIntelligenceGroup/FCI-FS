...
SAVE_FIGURES = True
SAVE_MATRICES = True
OUTPUT_DIR = Path(__file__).resolve().parent / "heatmaps2"
COLOR_GAMMA = 0.6
...
def extract_data_size(file_path):
	match = re.search(r"dataSize([0-9]*\.?[0-9]+)", file_path)
	if match is None:
		return None
	return float(match.group(1))
...
def format_data_size_label(data_size):
	if abs(data_size - round(data_size)) < 1e-9:
		return str(int(round(data_size)))
	return f"{data_size:g}"
...
def main():
	...
	for file_path in files:
		data_size = extract_data_size(file_path)
		...
		data_by_file.append((data_size, df, file_path))

	if len(data_by_file) == 0:
		print("No valid CSVs with dataSize values were loaded.")
		return
	...
	print(f"Loaded {len(data_by_file)} files with data sizes.")
	...
	for metric in METRICS:
		...
		for alg in algorithms:
			...
			for data_size, df, _ in data_by_file:
				row = [
			...
			metric_heatmaps.append((alg, x_labels, heatmap_data))
		...
		for alg, x_labels, heatmap_data in metric_heatmaps:
			observed_var_counts = [
			...
			ax.set_xticklabels([format_data_size_label(v) for v in x_labels], fontsize=TICK_LABEL_SIZE)
			...
			ax.set_xlabel("Data size", fontsize=AXIS_LABEL_SIZE)
			...
			base_name = safe_filename(f"heatmap2_{metric}_{alg}")

			if SAVE_MATRICES:
				matrix_exports.append((metric, display_alg, x_labels, observed_var_counts, heatmap_data))
	...
	if SAVE_MATRICES and len(matrix_exports) > 0:
		combined_txt_path = OUTPUT_DIR / "heatmap2_all_matrices.txt"
		save_all_matrices_txt(combined_txt_path, matrix_exports, "data size", format_data_size_label)
